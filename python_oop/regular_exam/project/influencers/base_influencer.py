    ...
    def display_campaigns_participated(self):
        ...
        if self.campaigns_participated:
            ...
            for campaign in self.campaigns_participated:
                followers = self.reached_followers(campaign.__class__.__name__)
                result += (f"  - Campaign ID: {campaign.campaign_id}, Brand: {campaign.brand}, Reached followers: "
                           f"{followers:.0f}\n")
        else:
            result += f"{self.username} has not participated in any campaigns."
